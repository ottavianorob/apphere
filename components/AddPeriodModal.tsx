...
interface AddPeriodModalProps {
  onClose: () => void;
  onSave: (name: string, start_year: number, end_year: number) => void;
}

const AddPeriodModal: React.FC<AddPeriodModalProps> = ({ onClose, onSave }) => {
      ...
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-[60] p-4 animate-fade-in" onClick={onClose}>
        <div className="bg-[#FAF7F0] w-full max-w-md max-h-[90vh] flex flex-col animate-slide-up border border-black/10 relative" onClick={(e) => e.stopPropagation()}>
            <header className="p-4 flex items-center justify-between border-b border-gray-300/80">
              <h2 className="font-sans-display text-2xl font-bold text-[#134A79]">Aggiungi Periodo Storico</h2>
            ...
            <div className="p-6 space-y-4">
               <div>
                  ...
                  <input id="period-name" type="text" value={name} onChange={e => setName(e.target.value)} className={inputStyle} required placeholder="Es. Boom Economico"/>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                    <label htmlFor="period-start" className={labelStyle}>Anno Inizio *</label>
                    <input id="period-start" type="number" value={startYear} onChange={e => setStartYear(e.target.value)} className={inputStyle} required placeholder="Es. 1946"/>
                </div>
                 <div>
                    <label htmlFor="period-end" className={labelStyle}>Anno Fine *</label>
                    <input id="period-end" type="number" value={endYear} onChange={e => setEndYear(e.target.value)} className={inputStyle} required placeholder="Es. 1959"/>
                </div>
              </div>
            </div>
            <footer className="p-4 border-t border-gray-300/80 flex justify-end gap-3">
                ...
                <button onClick={handleSubmit} className="px-4 py-2 text-white bg-[#134A79] font-sans-display font-semibold hover:bg-[#103a60] transition-colors rounded-md">Salva Periodo</button>
            </footer>
        </div>
