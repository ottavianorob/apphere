...
const ItineraryCard: React.FC<{ itinerary: Itinerary, points: Point[], onSelectPoint: (point: Point) => void }> = ({ itinerary, points, onSelectPoint }) => {
  return (
    <div className="border-b border-gray-300/80 py-8 mb-8">
      <h2 className="font-serif-display text-3xl font-bold text-[#134A79] mb-2">{itinerary.title}</h2>
      <p className="text-gray-700 mb-4 max-w-prose">{itinerary.description}</p>
      ...
      <div>
        <h3 className="font-serif-display text-xl font-bold text-gray-800 mb-3 border-b border-gray-300 pb-2">Tappe dell'itinerario</h3>
        <div className="space-y-1 mt-4">
          {points.map((point, index) => (
            ...
            >
              <div className="flex-shrink-0 bg-[#1C1C1C] text-[#EDE5D0] rounded-full h-8 w-8 flex items-center justify-center font-bold font-serif-display">
                {index + 1}
              </div>
              <div>
                <p className="font-bold text-gray-800 group-hover:text-[#134A79]">{point.title}</p>
                <p className="text-sm text-gray-600 line-clamp-1">{point.description}</p>
              </div>
            </div>
          ))}
        </div>
